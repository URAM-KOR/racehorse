...
# 휴대폰 핫스팟
# AcessKey: xYz9kHDxLdzaeDvISNU3NIQ4APTxhFFQsQ6rAI21
# SecretKey: xmUP96jyhkYxIkeuDq2MkPrbSFuOSwAdVS1pIBdW
...
#   1) 캔들 정보 배열
# -----------------------------------------------------------------------------
def get_candle(target_item, tick_kind, inq_range, to=""):
    try:
        ...
        while inq_range > 0:
            query_count = min(inq_range, page_size)
            querystring = {"market": target_item, "count": query_count, "to": to}
            res = send_request("GET", server_url + "/v1/candles/" + target_url, querystring, "")
            candle_data = res.json()

            logging.debug(candle_data)

            total_candle_data.extend(candle_data)

            # 다음 페이지가 있는지 확인
            if len(candle_data) < query_count:
                break

            # 다음 조회할 때 사용할 to 값 업데이트
            to = candle_data[-1]['candle_date_time_utc']

            inq_range -= query_count

        return total_candle_data
